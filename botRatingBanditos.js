(function(){
	...
	var Bot = { //объект бота1
		...
		sink: function() { //дневной слив
			...
			try {
				var id = parseInt($('#upl_list .hint:contains("'+my_nick+'")').parents()[1].id.replace(/\D+/g, ""));//лить в ответ
	...
	var Env = { //объект окружающая среда
		gam_id: 0,
		dvulfra: [],
		commands: [["Гражданин","Комиссар","Сержант","Доктор","Медработник","Смертник","Стерва","Вор","Свидетель","Дед Мороз","Валентин","Добрый зайка","Гринч","Влюблённый","Кондитер","Вредный зайка","Нефритовый заяц","Костюмер","Франкенштейн","Привидение","Дракула","Бешеный пират"], ["Мафиози","Босс мафии","Двуликий"], ["Маньяк"], ["Валентин"], ["Чокнутый Профессор","Зомби"], ["Потрошитель"], ["Подручный","Жирный Тони","Марко","Франческа","Розарио","Хулиганка Пеппи"], ["Тётушка Лин","Якудза","Гора","Тень"],["Чёрная Борода"],["Санчо", "Мигель", "Бандит"]],
		idcommands: { 1: [1,4,5,6,7,8,10,11,12,13,15,20,21,22,23,24,26,27,32,28,39], 2: [2,9,25], 3: [3], 4: [14], 5: [18,19], 6: [34], 7: [16,17,30,31,33,41], 8: [35,36,37,38], 9: [40], 10: [24, 21, 43] },
				...
				if (!pla_data['dead']){
					...
					if (gam_data['v_mode']){ //день
						if (!pla_data["freeze"]){ //если не в морозе
							...
							} else if (!pla_data['act']){ //дневной слив (!$('li .x2vote, li .reanim, li .z_ghost').length)
								Bot.sink();
							}
						}
